...
Camera::Camera(scene::ISceneManager* smgr, MapDrawControl& draw_control,
	...
	m_camera_mode(CAMERA_MODE_THIRD)//CAMERA_MODE_FIRST
{
		...
		f32 tool_reload_ratio, ClientEnvironment &c_env)
{
	...
	if (m_camera_mode == CAMERA_EGO)
	{
		...
		// Calculate new position
		bool abort = false;
		for (int i = BS; i <= BS*15.75; i++)
		{
			my_cp.X = 0;//m_camera_position.X;// + m_camera_direction.X*-i;
			my_cp.Z = 100;//m_camera_position.Z;// + m_camera_direction.Z*-i;
